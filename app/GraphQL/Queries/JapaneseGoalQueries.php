    ...
    public function myDiary($_,array $args){
        ...
        if ($args["type"] == "diary") 
        {
            ...
            foreach($diary as $value){
                ...
                {
                    $more = array_intersect_key($v , array_flip(['user_invite_ids', 'date']));
                ...
                {
                    $result->push(["id" => $value->id, "user" => ['id' => $value->user_id,'name' => $value->User->name],"goal_name"=>$nameGoals[$value->goal_id]['name'], "more"=>$more]);
                }else
